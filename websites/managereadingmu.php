...
<html lang="en">
   ...
   <body onload="loadLanguage()">
      <div class="container-narrow">
         ...
         <hr>
         <div id="manageall">
       <a class="btn btn-default" style="margin:10px;" href="manageread_multi.php" role="button">Manage Fill Blank</a>
       ...
       <h1>Manage Reading multi (wanxing)</h1><br>
            
                  <div id="read_multitable">
                     <?php
                        $db = mysql_connect('localhost','root','password')
                         or die('Error connecting to MySQL server.');
                         @mysql_select_db("test", $db);
                         $sql = "SELECT * FROM test.read_multi";
                     ...
                     <?php
                        $count = "SELECT COUNT(idread_multi) FROM test.read_multi";
                           ...
                           <tr>
                              ...
                              <td>
                                 ...
                                 <!-- <?php
                                    if (isset($_GET['idd'])){
                                    $idd = $_GET['idd'];
                                    $res = mysql_query("DELETE FROM test.read_multi WHERE idnew_table='$idd'");
                     ...
                     <?php
                        ...
                        $iddd = $_POST['idd'];
                        $res = mysql_query("DELETE FROM test.read_multi WHERE idread_multi='$iddd'");
                        mysql_query(" ALTER TABLE test.read_multi DROP `idread_multi`");
                        mysql_query("ALTER TABLE test.read_multi  AUTO_INCREMENT = 1");
                        mysql_query("ALTER TABLE test.read_multi  ADD `idread_multi` int UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY FIRST");
                        if(! $res){
                        die('Could not update data: ' . mysql_error());
                        }
                        echo "Updated data successfully\n";
                        echo '<script> window.location.href="manageread_multi.php"; </script>';
                        }
                           ?>
                     <?php
      ...
      <!--
         ...
         var idread_multi = $(this).attr("id");
         $.ajax({
         url:"fetch.php",
         method: "POST",
         data:{idread_multi:idread_multi},
         dataType:"json",
         ...
         $('#date').val(data.date);
         $('#idread_multi').val(data.idread_multi);
         $('#insert').val("Update");
         $('#myModal').modal('show');
         }
