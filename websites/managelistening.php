   ...
   <body onload="loadLanguage()">
      <div class="container-narrow">
         ...
         <div id="sepmanagepage">
           <div id="manageall">
       <a class="btn btn-default" style="margin:10px;" href="managelistening.php" role="button">Manage Fill Blank</a>
       ...
       <hr>
       <h1 class="managepagetitle">Manage Listening</h1><br>
            
                  <div id="listeningtable">
                     <?php
                        $db = mysql_connect('localhost','root','password')
                         or die('Error connecting to MySQL server.');
                         @mysql_select_db("test", $db);
                         $sql = "SELECT * FROM test.listening";
                     ...
                     <?php
                        $count = "SELECT COUNT(idlistening) FROM test.listening";
                  ...
                  echo ' questions </span>'; ?>
                           ...
                           <tr>
                              ...
                              <hr>
                                 ...
                                 <!-- <?php
                                    if (isset($_GET['idd'])){
                                    $idd = $_GET['idd'];
                                    $res = mysql_query("DELETE FROM test.listening WHERE idnew_table='$idd'");
                     ...
                     <?php
                        ...
                        $iddd = $_POST['idd'];
                        $res = mysql_query("DELETE FROM test.listening WHERE idlistening='$iddd'");
                        mysql_query(" ALTER TABLE test.listening DROP `idlistening`");
                        mysql_query("ALTER TABLE test.listening  AUTO_INCREMENT = 1");
                        mysql_query("ALTER TABLE test.listening  ADD `idlistening` int UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY FIRST");
                        if(! $res){
                        die('Could not update data: ' . mysql_error());
                        }
                        echo "Updated data successfully\n";
                        echo '<script> window.location.href="managelistening.php"; </script>';
                        }
                           ?>
                     <?php
      ...
      <!--
         ...
         var idlistening = $(this).attr("id");
         $.ajax({
         url:"fetch.php",
         method: "POST",
         data:{idlistening:idlistening},
         dataType:"json",
         ...
         $('#date').val(data.date);
         $('#idlistening').val(data.idlistening);
         $('#insert').val("Update");
         $('#myModal').modal('show');
         }
